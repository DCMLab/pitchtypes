...
class TestValueTables(TestCase):
    ...
    def test_value_tables(self):
        ...
        operation_regex = re.compile(
            "^(?P<type1>Pitch|Interval|PitchClass|IntervalClass)"
            "_(?P<operation>.+)_"
        ...
        for folder in os.listdir(base_dir):
            ...
            type_checks = {"Pitch": False, "Interval": False, "PitchClass": False, "IntervalClass": False}
            # remember what operation checks have been done
            operation_checks = {
                "Pitch_plus_Interval": False,
            ...
            for table in os.listdir(os.path.join(base_dir, folder)):
                ...
                arr = np.loadtxt(
                    os.path.join(base_dir, folder, table), ndmin=2, dtype=str, delimiter="\t", comments=None
                )
                ...
                if type_match is not None:
                    ...
                    for idx in range(arr.shape[0]):
                        ...
                        if obj.is_interval:
                            ...
                            try:
                                ...
                                print(val, print_val, print_inv, obj, obj.value, -obj, (-obj).value)
                                print(neg_obj, neg_obj.value, neg_obj_, neg_obj_.value)
                                raise
                        else:
                            print_val = val
                        # check class
                    ...
                    for idx_1 in range(1, arr.shape[0]):
                        ...
                        obj_1 = self.make_obj(base_type=base_type, sub_type=op_match["type1"], val=val_1)
                        ...
                        for idx_2 in range(1, arr.shape[1]):
                            ...
                            obj_2 = self.make_obj(base_type=base_type, sub_type=op_match["type2"], val=val_2)
                            ...
                            try:
                                ...
                                print(
                                    f"{obj_1} {type(obj_1)} {operation} {obj_2} {type(obj_2)} == {res} {type(res)} "
                                    f"(should be {val_res})"
                                )
                    ...
                    self.fail()
            # make sure all subtypes were checked
