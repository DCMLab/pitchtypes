...
class TestValueTables(TestCase):
    ...
    def test_value_tables(self):
        ...
        base_dir = os.path.join(tests_directory, "value_tables")
        valid_folders = False
        for folder in os.listdir(base_dir):
            ...
            for table in os.listdir(os.path.join(base_dir, folder)):
                ...
                arr = np.loadtxt(
                    os.path.join(base_dir, folder, table),
                    ndmin=2,
                    dtype=str,
                    delimiter="\t",
                    comments=None,
                ...
                if type_match is not None:
                    ...
                    for idx in range(arr.shape[0]):
                        ...
                        # get the object
                        obj = self.make_obj(
                            base_type=base_type, sub_type=type_match["type"], val=val
                        )
                        ...
                        if obj.is_interval:
                            ...
                            if val.startswith("-"):
                                ...
                                neg_val = "-" + val
                            neg_obj_ = self.make_obj(
                                base_type=base_type,
                                sub_type=type_match["type"],
                                val=neg_val,
                            )
                            # construct from string in value table
                            neg_obj = self.make_obj(
                                base_type=base_type,
                                sub_type=type_match["type"],
                                val=print_inv,
                            )
                    ...
                    for idx_1 in range(1, arr.shape[0]):
                        ...
                        obj_1 = self.make_obj(
                            base_type=base_type, sub_type=op_match["type1"], val=val_1
                        )
            ...
            if not np.all(list(type_checks.values())):
                unchecked_types = "".join(
                    f"\n    {key}" for key, val in type_checks.items() if not val
                )
                self.fail(
                    f"For base type {folder} some types were not checked:{unchecked_types}"
                )
            ...
            if not np.all(list(operation_checks.values())):
                ...
                self.fail(
                    f"For base type {folder} some operations were not checked:{unchecked_ops}"
                )
            if not valid_tables:
