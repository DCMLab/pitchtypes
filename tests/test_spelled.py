...
class TestSpelled(TestCase):
        ...
        for is_class in [True, False]:
            ...
            for idx, p in enumerate(self.line_of_fifths):
                if is_class:
                    ...
                    for octave in range(-3, 10):
                        p_oct = p + str(octave)
                        pp = SpelledPitch(p_oct)
                        # test factory functions
                        self.assertEqual(
                            pp, SpelledPitch.from_fifths_and_octaves(fifths=pp.fifths(), octaves=pp.internal_octaves())
                        )
                        # check conversion to enharmonic
            ...
            for idx, (interval_class_str, inverse_interval_class_str) in enumerate(
                ...
                if is_class:
                    ...
                    # check conversion to enharmonic
                    self.assertEqual(
                        interval.convert_to(Enharmonic.IntervalClass), Enharmonic.IntervalClass(interval_class_str)
                    )
                    # test class conversion
                    ...
                    for octave in range(0, 10):
                        ...
                        self.assertEqual(
                            interval.octaves(),
                            interval.value[0] + interval._diatonic_steps_from_fifths(interval.fifths()) // 7,
                        )
                        ...
                        self.assertEqual(
                            ...
                            SpelledInterval.from_fifths_and_octaves(
                                fifths=interval.fifths(), octaves=interval.internal_octaves()
                            ),
                        )
                        # check conversion to enharmonic
    ...
    def test_from_fifths_functions(self):
        ...
        for fifths, diatonic, interval_class, inverse_interval_class in [
            ...
            self.assertEqual(diatonic, Spelled._diatonic_steps_from_fifths(fifths))
            self.assertEqual(diatonic % 7 + 1, Spelled._generic_interval_class_from_fifths(fifths))
    ...
    def test_parsing(self):
        ...
        # check bad input
        self.assertRaises(ValueError, lambda: Spelled._fifths_from_diatonic_pitch_class("X"))
        self.assertRaises(ValueError, lambda: Spelled._fifths_from_generic_interval_class("X"))
    ...
    def test_onehot(self):
        ...
        self.assertEqual(
            SpelledInterval.from_onehot(SpelledInterval("a4:2").onehot((-8, 8), (-2, 2)), -8, -2),
            SpelledInterval("a4:2"),
        )
        self.assertRaises(ValueError, lambda: SpelledInterval.from_onehot(np.array([1, 0, 1]), 0, 0))

        self.arrayEqual(SpelledIntervalClass("M2").onehot((-2, 3)), np.array([0, 0, 0, 0, 1, 0]))
        self.assertRaises(ValueError, lambda: SpelledIntervalClass("M6").onehot((-2, 2)))
        self.assertEqual(
            SpelledIntervalClass.from_onehot(SpelledIntervalClass("a4").onehot((-8, 8)), -8), SpelledIntervalClass("a4")
        )
        ...
        self.assertEqual(
            SpelledPitch.from_onehot(SpelledPitch("F#4").onehot((-8, 8), (0, 6)), -8, 0), SpelledPitch("F#4")
        )
        self.assertRaises(ValueError, lambda: SpelledPitch.from_onehot(np.array([1, 0, 1]), 0, 0))

        self.arrayEqual(SpelledPitchClass("D").onehot((-2, 3)), np.array([0, 0, 0, 0, 1, 0]))
        self.assertRaises(ValueError, lambda: SpelledPitchClass("A").onehot((-2, 2)))
        self.assertEqual(
            SpelledPitchClass.from_onehot(SpelledPitchClass("F#").onehot((-8, 8)), -8), SpelledPitchClass("F#")
        )
