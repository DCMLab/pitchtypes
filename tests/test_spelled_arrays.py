...
class TestSpelledArray(TestCase):
    ...
    def test_constructors(self):
        self.spelledEqual(
            SpelledIntervalArray(np.arange(5) - 2, np.arange(5) - 2), asi(["-M2:3", "-P5:1", "P1:0", "P5:1", "M2:3"])
        )
        self.spelledEqual(SpelledIntervalClassArray(np.arange(5)), asic(["P1", "P5", "M2", "M6", "M3"]))
        self.spelledEqual(SpelledPitchArray(np.arange(5) - 2, 4 - np.arange(5)), asp(["Bb2", "F2", "C2", "G1", "D1"]))
        self.spelledEqual(SpelledPitchClassArray(np.arange(5) - 2), aspc(["Bb", "F", "C", "G", "D"]))

        self.spelledEqual(
            SpelledIntervalArray.from_independent(np.arange(5) - 2, np.zeros(5, dtype=np.int_)),
            asi(["m7:0", "P4:0", "P1:0", "P5:0", "M2:0"]),
        )
        self.spelledEqual(
            SpelledPitchArray.from_independent(np.arange(5) - 2, 4 - np.arange(5)), asp(["Bb4", "F3", "C2", "G1", "D0"])
        )
    ...
    def test_intervals(self):
        ...
        self.spelledEqual(
            asi(["m3:0", "m3:0", "P5:0", "-m3:0", "m3:0"]) + asi(["M3:0", "M7:0", "P5:0", "M3:0", "-M3:0"]),
            asi(["P5:0", "M2:1", "M2:1", "a1:0", "-a1:0"]),
        )
        self.spelledEqual(asi(["m3:0", "m3:0"]) - asi(["M3:0", "M6:0"]), asi(["-a1:0", "-a4:0"]))
        self.spelledEqual(-asi(["P4:0", "P4:0", "P5:0"]), asi(["-P4:0", "P5:-1", "-P5:0"]))
        ...
        self.spelledEqual(
            asi(["P5:0", "M2:0", "-m3:0", "M3:0", "M2:0", "-M3:0"]) * np.array([2, 4, 4, -3, 4, 4]),
            asi(["M2:1", "a5:0", "-d2:1", "-a7:0", "a5:0", "-aa2:1"]),
        )
        self.spelledEqual(5 * asi(["M3:0"]), asi(["aaa4:1"]))

        self.arrayEqual(asi(["m2:0", "P1:0", "d1:0", "a1:0", "-m3:0"]).direction(), [1, 0, -1, 1, -1])
        self.spelledEqual(
            asi(["m2:0", "P1:0", "d1:0", "a1:0", "-m3:0"]).abs(), asi(["m2:0", "P1:0", "a1:0", "a1:0", "m3:0"])
        )
        ...
        self.assertTrue(
            asi(["d1:0", "P1:0", "a1:0", "d2:0", "m2:0", "M2:0", "a2:0", "-d2:0", "-m2:0", "-M2:0", "-a2:0"])
            .is_step()
            .all()
        )
        self.assertFalse(asi(["d3:0", "-d3:0", "M7:0", "-M7:0", "P1:1", "-P1:1", "m2:1", "-m2:1"]).is_step().any())
    ...
    def test_ics(self):
        ...
        self.spelledEqual(
            asic(["m3", "m3", "P5", "-m3", "m3"]) + asic(["M3", "M7", "P5", "M3", "-M3"]),
            asic(["P5", "M2", "M2", "a1", "-a1"]),
        )
        self.spelledEqual(asic(["m3", "m3"]) - asic(["M3", "M6"]), asic(["-a1", "-a4"]))
        self.spelledEqual(-asic(["P4", "P4", "P5"]), asic(["-P4", "P5", "-P5"]))
        ...
        self.spelledEqual(
            asic(["P5", "M2", "-m3", "M3", "M2", "-M3"]) * np.array([2, 4, 4, -3, 4, 4]),
            asic(["M2", "a5", "-d2", "-a7", "a5", "-aa2"]),
        )
        self.spelledEqual(5 * asic(["M3"]), asic(["aaa4"]))

        self.arrayEqual(asic(["m2", "P1", "d1", "a1", "-m3"]).direction(), [1, 0, -1, 1, -1])
        self.spelledEqual(asic(["m2", "P1", "d1", "a1", "-m3"]).abs(), asic(["m2", "P1", "a1", "a1", "m3"]))
        ...
        self.assertTrue(
            asic(["d1", "P1", "a1", "d2", "m2", "M2", "a2", "-d2", "-m2", "-M2", "-a2", "M7", "-M7"]).is_step().all()
        )
        self.assertFalse(asic(["d3", "-d3"]).is_step().any())

    def test_pitches(self):
        self.spelledEqual(asp(["Eb4", "Eb4"]) + asi(["P5:0", "-m3:0"]), asp(["Bb4", "C4"]))
        self.spelledEqual(asp(["Eb4", "Eb4"]) - asi(["P5:0", "-m3:0"]), asp(["Ab3", "Gb4"]))
        self.spelledEqual(asp(["G4", "Eb4"]) - asp(["C#4", "G4"]), asi(["d5:0", "-M3:0"]))
        ...
        self.spelledEqual(asp(["Ab-1", "Eb4", "D##100"]).embed(), asp(["Ab-1", "Eb4", "D##100"]))

    def test_pcs(self):
        self.spelledEqual(aspc(["Eb", "Eb"]) + asic(["P5", "-m3"]), aspc(["Bb", "C"]))
        self.spelledEqual(aspc(["Eb", "Eb"]) - asic(["P5", "-m3"]), aspc(["Ab", "Gb"]))
        self.spelledEqual(aspc(["G", "Eb"]) - aspc(["C#", "G"]), asic(["d5", "-M3"]))
        ...
        self.spelledEqual(aspc(["Ab", "Eb", "D##"]).embed(), asp(["Ab0", "Eb0", "D##0"]))
    ...
    def test_comparison(self):
        ...
        self.arrayEqual(asi(["d5:0", "P1:0", "-M2:0"]) > asi(["a4:0", "a1:0", "-m2:0"]), [True, False, False])
        self.arrayEqual(asi(["d5:0", "P1:0", "-M2:0"]) < asi(["a4:0", "a1:0", "-m2:0"]), [False, True, True])
        self.arrayEqual(
            asi(["m2:0", "a1:0", "P1:0", "-a1:0", "-m2:0"]) > SpelledInterval("P1:0"), [True, True, False, False, False]
        )
        self.arrayEqual(
            asi(["m2:0", "a1:0", "P1:0", "-a1:0", "-m2:0"]) < SpelledInterval("P1:0"), [False, False, False, True, True]
        )
        self.arrayEqual(
            asi(["m2:0", "a1:0", "P1:0", "-a1:0", "-m2:0"]) >= SpelledInterval("P1:0"), [True, True, True, False, False]
        )
        self.arrayEqual(
            asi(["m2:0", "a1:0", "P1:0", "-a1:0", "-m2:0"]) <= SpelledInterval("P1:0"), [False, False, True, True, True]
        )
        self.arrayEqual(
            asi(["m2:0", "a1:0", "P1:0", "-a1:0", "-m2:0"]) == SpelledInterval("P1:0"),
            [False, False, True, False, False],
        )
        self.arrayEqual(
            asi(["m2:0", "a1:0", "P1:0", "-a1:0", "-m2:0"]) != SpelledInterval("P1:0"), [True, True, False, True, True]
        )
        ...
        self.arrayEqual(asp(["Gb4", "C0", "Bb-1"]) > asp(["F#4", "C#0", "B-1"]), [True, False, False])
        self.arrayEqual(asp(["Gb4", "C0", "Bb-1"]) < asp(["F#4", "C#0", "B-1"]), [False, True, True])
        self.arrayEqual(asp(["Db4", "C#4", "C4", "Cb4", "B3"]) > SpelledPitch("C4"), [True, True, False, False, False])
        self.arrayEqual(
            asp(["Db4", "C#4", "C4", "Cb4", "B3"]) == SpelledPitch("C4"), [False, False, True, False, False]
        )

        # interval/pitch class ordering uses the line of fifths
        self.arrayEqual(asic(["d5", "P1", "-M2"]) > asic(["a4", "a1", "-m2"]), [False, False, False])
        self.arrayEqual(asic(["d5", "P1", "-M2"]) < asic(["a4", "a1", "-m2"]), [True, True, True])
        self.arrayEqual(
            asic(["m2", "a1", "P1", "-a1", "-m2"]) > SpelledIntervalClass("P1"), [False, True, False, False, True]
        )
        self.arrayEqual(
            asic(["m2", "a1", "P1", "-a1", "-m2"]) == SpelledIntervalClass("P1"), [False, False, True, False, False]
        )

        self.arrayEqual(aspc(["Gb", "C", "Bb"]) > aspc(["F#", "C#", "B"]), [False, False, False])
        self.arrayEqual(aspc(["Gb", "C", "Bb"]) < aspc(["F#", "C#", "B"]), [True, True, True])
        self.arrayEqual(aspc(["Db", "C#", "C", "Cb", "B"]) > SpelledPitchClass("C"), [False, True, False, False, True])
        self.arrayEqual(
            aspc(["Db", "C#", "C", "Cb", "B"]) == SpelledPitchClass("C"), [False, False, True, False, False]
        )

    def test_onehot(self):
        i = asi([["M6:0", "m3:0", "M2:1"], ["-M6:0", "-m3:0", "P1:0"]])
